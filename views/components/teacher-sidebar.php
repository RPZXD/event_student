...
<aside id="sidebar" class="fixed top-0 left-0 z-40 w-72 sm:w-64 h-screen transition-transform duration-300 ease-in-out -translate-x-full lg:translate-x-0">
    <!-- Sidebar Background -->
    <div class="h-full overflow-y-auto bg-gradient-to-b from-indigo-800 via-indigo-900 to-slate-950 dark:from-slate-900 dark:via-slate-950 dark:to-black">
            ...
            <div class="flex items-center justify-between">
                <a href="index.php" class="flex items-center space-x-3 group flex-1 min-w-0">
                    <div class="relative flex-shrink-0">
                        ...
                        <img src="../dist/img/<?php echo $global['logoLink'] ?? 'logo-phicha.png'; ?>" class="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full ring-2 ring-white/20 group-hover:ring-indigo-400 transition-all" alt="Logo">
                    </div>
                    <div class="min-w-0">
                        <span class="text-base sm:text-lg font-bold text-white truncate block">Teacher Portal</span>
                        <p class="text-xs text-indigo-300 truncate">ระบบจัดการกิจกรรม</p>
                    </div>
                </a>
        ...
        <?php if (isset($_SESSION['username'])): 
            ...
            $teacherName = isset($_SESSION['user']['Teach_name']) ? $_SESSION['user']['Teach_name'] : $_SESSION['username'];
        ?>
        <div class="px-4 sm:px-6 py-4 border-b border-white/10">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                    👨‍🏫
                </div>
                <div class="min-w-0 flex-1">
                    <p class="text-sm font-medium text-white truncate"><?php echo htmlspecialchars($teacherName); ?></p>
                    <p class="text-xs text-indigo-300">ครู/อาจารย์</p>
                </div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <nav class="mt-4 sm:mt-6 px-2 sm:px-3 pb-20">
            <p class="px-4 mb-3 text-xs font-semibold text-indigo-400 uppercase tracking-wider">เมนูหลัก</p>
            <ul class="space-y-1">
                <!-- Main Menu Items -->
